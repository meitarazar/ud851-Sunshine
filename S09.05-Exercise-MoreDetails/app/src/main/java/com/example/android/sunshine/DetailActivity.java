...
//  COMPLETED (19) Create constant int values representing each column name's position above
    ...
    private static final int INDEX_COLUMN_WIND_SPEED = 5;
    private static final int INDEX_COLUMN_DEGREES = 6;
    private static final int INDEX_COLUMN_PRESSURE = 7;

//  COMPLETED (20) Create a constant int to identify our loader used in DetailActivity
    private static final int DATA_LOADER_ID = 15965;
...
//  COMPLETED (10) Remove the mWeatherDisplay TextView declaration
    ...
    private TextView mDisplayTempHigh;
    private TextView mDisplayTempLow;
    private TextView mDisplayHumidity;
    private TextView mDisplayWind;
...
//      COMPLETED (13) Find each of the TextViews by ID
        mDisplayDate = (TextView) findViewById(R.id.tv_display_day);
        mDisplayDescription = (TextView) findViewById(R.id.tv_display_description);
        mDisplayTempHigh = (TextView) findViewById(R.id.tv_display_temperature_high);
        mDisplayTempLow = (TextView) findViewById(R.id.tv_display_temperature_low);
        mDisplayHumidity = (TextView) findViewById(R.id.tv_display_humidity);
        mDisplayWind = (TextView) findViewById(R.id.tv_display_wind);
        mDisplayPressure = (TextView) findViewById(R.id.tv_display_pressure);
...
//      COMPLETED (35) Initialize the loader for DetailActivity
        if (intentThatStartedThisActivity != null) {
            ...
            if (mUri == null) {
                throw new NullPointerException("Passed URI must not be null!");
            }
...
//      COMPLETED (33) Store a forecast summary in mForecastSummary
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        String dateFriendly = SunshineDateUtils.getFriendlyDateString(this, date, true);
        mDisplayDate.setText(dateFriendly);

        int id = data.getInt(INDEX_COLUMN_WEATHER_ID);
        String description = SunshineWeatherUtils.getStringForWeatherCondition(this, id);
        mDisplayDescription.setText(description);

        double high = data.getDouble(INDEX_COLUMN_MAX_TEMP);
        String highString = SunshineWeatherUtils.formatTemperature(this, high);
        mDisplayTempHigh.setText(highString);

        double low = data.getDouble(INDEX_COLUMN_MIN_TEMP);
        String lowString = SunshineWeatherUtils.formatTemperature(this, low);
        mDisplayTempLow.setText(lowString);

        float humidity = data.getFloat(INDEX_COLUMN_HUMIDITY);
        mDisplayHumidity.setText(getString(R.string.format_humidity, humidity));

        float speed = data.getFloat(INDEX_COLUMN_WIND_SPEED);
        float direction = data.getFloat(INDEX_COLUMN_DEGREES);
        mDisplayWind.setText(SunshineWeatherUtils.getFormattedWind(this, speed, direction));

        float pressure = data.getFloat(INDEX_COLUMN_PRESSURE);
        mDisplayPressure.setText(getString(R.string.format_pressure, pressure));

        mForecastSummary = String.format("%s - %s\n%s / %s", dateFriendly, description, highString, lowString);
    }
